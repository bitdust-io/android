...
public class SDLActivity extends Activity implements View.OnSystemUiVisibilityChangeListener {
    ...
    protected void finishLoad() {
        ...
        //Log.v(TAG, "onCreate() will not acquire HIDDeviceManager");
    ...
    protected void onDestroy() {
        ...
        try {
            if (mHIDDeviceManager != null) {
                Log.v(TAG, "onDestroy() about to release mHIDDeviceManager");
                HIDDeviceManager.release(mHIDDeviceManager);
                mHIDDeviceManager = null;
            }

            if (SDLActivity.mBrokenLibraries) {
                Log.v(TAG, "onDestroy() mBrokenLibraries is TRUE");
                super.onDestroy();
                // Reset everything in case the user re opens the app
                SDLActivity.initialize();
                Log.v(TAG, "onDestroy() is finished");
                return;
            }

            Log.v(TAG, "onDestroy() about to call handleNativeState");
            mNextNativeState = NativeState.PAUSED;
            SDLActivity.handleNativeState();

            // Send a quit message to the application
            Log.v(TAG, "onDestroy() about to call nativeQuit");
            ...
            for (int i = 0; i < noThreads; i++) {
                Log.v(TAG, "Thread No:" + i + " = " + lstThreads[i].getName());
            }
            ...
            if (SDLActivity.mSDLThread != null) {
                Log.v(TAG, "onDestroy() will wait mSDLThread to finish");
                //try {
            ...
            Log.v(TAG, "onDestroy() thread stopped");
...
class SDLMain implements Runnable {
    ...
    public void run() {
        ...
        if (!SDLActivity.mExitCalledFromJava) {
            Log.v("SDL", "about to call handleNativeExit()");
            SDLActivity.handleNativeExit();
            Log.v("SDL", "handleNativeExit() done");
        }
    }
}
